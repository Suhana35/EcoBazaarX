...
const ProductDetails = () => {
  ...
  // Fetch smart recommendations; fall back to any other available products
  useEffect(() => {
    if (!product) return;
    ...
    getRecommendations(product.id, 4)
      .then((res) => {
        ...
        if (data.length > 0) {
          ...
          // No same-category matches — show any other available products
          setRelatedProducts(
      ...
      .catch(() => {
        // API failed — fall back to other available products from context
        setRelatedProducts(
          products.filter((p) => p.id !== product.id).slice(0, 4)
        );
      });
  ...
  const materialCO2   = product.materialCO2  || 0;
  const shippingCO2   = product.shippingCO2  || 0;
  // Use stored footprint if present, otherwise derive from components
  const totalFootprint = product.footprint
    ? parseFloat(product.footprint)
    : materialCO2 + shippingCO2;
  // Guard against division-by-zero when both values are 0 (Infinity% → 0%)
  const materialPercentage = totalFootprint > 0 ? (materialCO2 / totalFootprint) * 100 : 0;
  const shippingPercentage = totalFootprint > 0 ? (shippingCO2 / totalFootprint) * 100 : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-blue-100">
      <div className="container mx-auto px-4 py-8 max-w-6xl">
              ...
              <div className="space-y-4">
                <div className="bg-orange-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="font-semibold text-orange-700">{materialCO2.toFixed(2)} kg CO₂</span>
                  </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="font-semibold text-blue-700">{shippingCO2.toFixed(2)} kg CO₂</span>
                  </div>
        ...
        {/* Related Products — powered by smart recommendations */}
