...
const ProductModal = ({ 
  ...
  const [imageInputMode, setImageInputMode] = useState("url"); // "url" | "device"
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditMode && product) {
        ...
        setImageInputMode(product.image?.startsWith("data:") ? "device" : "url");
      } else {
        setFormData({
          name: "",
          type: "",
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            {/* Tab Switcher */}
            <div className="flex rounded-2xl border border-gray-200 overflow-hidden mb-4 w-fit">
              <button
                type="button"
                onClick={() => {
                  setImageInputMode("url");
                ...
                className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold transition-colors duration-200 ${
                  imageInputMode === "url"
            ...
            {imageInputMode === "url" && (
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-gray-500">Paste a direct link to your product image</p>
              </div>
            )}
            ...
            {imageInputMode === "device" && (
              <div className="space-y-2">
                ...
                >
                  {uploadingImage ? (
                    <>
                      ...
                      <p className="text-sm text-sky-600 font-medium">Uploading image...</p>
                    </>
                  ) : (
                    <>
                      <div className="w-12 h-12 rounded-full bg-sky-100 flex items-center justify-center">
                        <FiUpload size={22} className="text-sky-500" />
                      </div>
                ...
                <input
                  ...
                  className="hidden"
                />
                <p className="text-xs text-gray-500">Supported formats: JPG, PNG, GIF, WEBP &mdash; max 5MB</p>
              </div>
            )}

            {formErrors.image && <p className="text-red-500 text-sm mt-2">{formErrors.image}</p>}

            {/* Image Preview */}
            {imagePreview && (
              <div className="mt-4 flex items-start gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-200">
                ...
                <div className="flex flex-col justify-center gap-1">
                  ...
                  <p className="text-xs text-gray-500">
                    {formData.image?.startsWith("data:") ? "Uploaded from device" : "From URL"}
                  </p>
